...
#' 
#' fit.perc.assert <- function(p, q, fit.weights, show.output, tolPlot, tolConv) {
...
#'   fit.perc.assert(p, q, fit.weights, show.output, tolPlot, tolConv)
#'     # if (length(p) != length(q) | 
#'     #     length(p) != length(fit.weights) | 
#'     #     length(q) != length(fit.weights)) {
...
plotDiagnostics.perc <- function(fit.results, tolPlot = 0.1) {
  cat("plotDiagnostics.perc\n")
    ...
    #-----------------------------------------------------------------------------
    graphics::plot(p ~ q, type = "p", 
                   cex = 1.5, pch = 19, lwd = 2, 
                   xlim = c(min, max),
                   ylim = c(0, 1),
                   main = "Graphical diagnostics",
                   xlab = "Percentile", 
    ...
    if (!is.na(res.mat$chisqnc[1])) {
        test.quantiles <- suppressWarnings(stats::qchisq(p, 
                                                         df = res.mat$chisqnc[1], 
    ...
    if (!is.na(res.mat$chisq[1])) {
        ...
        try(if (sum(abs(test.quantiles - q)) < tolPlot) {
            graphics::lines(stats::pchisq(xx, 
                                          df = res.mat$chisq[1]) ~ xx, 
...
get.beta.par <- function(p = c(0.025, 0.5, 0.975), q, 
    ...
    if (prod(!is.na(Par)) & plot) {
        ...
        main <- paste0("Beta (", main1, ", ", main2, ")")
        sub = paste0("fit.weights = c(", paste(fit.weights.original, collapse = ", "), ")")
...
fit.cont <- function(data2fit = stats::rnorm(1000), continuousFunction) {
    ...
    onOK <- function(...) {
      ...
      tempEnvir$chosenD <- tcltk::tclvalue(rbValue)
      ...
      tempEnvir$fittedParams <- fit.list[[which(names(fit.list) == tcltk::tclvalue(rbValue))]]$estimate
...
rriskMMEdist <- function(data, distr) {
  ...
  if (!(is.vector(data) & is.numeric(data) & length(data) > 1))
    stop(paste("ERROR in func 'rriskMMEdist':", 
               "Input data must be a numerical vector of length greater than 1."))
    
  if (distname == "norm") {
    n <- length(data)
    sd0 <- sqrt((n - 1)/n) * stats::sd(data) # ? se?
    mx <- mean(data)
    estimate <- c(mx, sd0)
    ...
    stop(paste("ERROR in 'rriskMMEdist':", 
               "Method of matching moments is not available for ",
